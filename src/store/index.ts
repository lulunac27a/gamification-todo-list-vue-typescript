...
export default createStore({
  ...
  mutations: {
    ...
    updateXp: (state, payload) => {
      ...
      const daysToDue: number = (Number(new Date(task.dueDate + " 23:59:59.999")) - Number(new Date().setHours(23, 59, 59, 999))) / (1000 * 60 * 60 * 24);//calculate number of days until the task is due
      const dateMultiplier: number = daysToDue < 0 ? -2 / (daysToDue - 1) : daysToDue === 0 ? 4 / (1 + (Number(new Date().setHours(23, 59, 59, 999)) - Number(new Date())) / (1000 * 24 * 60 * 60)) : 1 + 1 / (daysToDue + 1);//if task is overdue, XP and score multiplier is less than 1 that decreases over time when task is overdue, else XP multiplier bonus increases (more than 1) when task gets closer to due date
      ...
      const activeTasks: number = state.todos.filter((taskList) => !taskList.completed).length;//calculate number of active tasks (tasks that are not completed) using array.filter
      ...
      if (Number(task.repeatFrequency) === 1) {//if task repetition is daily
        if (task.repeatOften < 7) {//7 days is 1 week
        ...
        } else if (task.repeatOften < 30) {//approximately 30 days is 1 month
        ...
        } else if (task.repeatOften < 365) {//approximately 365 days is 1 year
      ...
      } else if (Number(task.repeatFrequency) === 2) {//if task repetition is weekly
        if (task.repeatOften < 4) {//approximately 4 weeks is 1 month
        ...
        } else if (task.repeatOften < 52) {//approximately 52 weeks is 1 year
      ...
      } else if (Number(task.repeatFrequency) === 3) {//if task repetition is monthly
        if (task.repeatOften < 12) {//12 months is 1 year
      ...
      else {//if task is completed before due date (not overdue)
      ...
      else if (state.user.dailyStreak < 14) {//2 weeks is 14 days
      ...
      else if (state.user.dailyStreak < 60) {//2 months is approximately 60 days
      ...
      else if (state.user.dailyStreak < 90) {//3 months is approximately 90 days
      ...
      else if (state.user.dailyStreak < 180) {//6 months is approximately 180 days
      ...
      else if (state.user.dailyStreak < 730) {//2 years is approximately 730 days
      ...
      else if (state.user.dailyStreak < 1461) {//4 years is approximately 1,461 days
      ...
      else if (state.user.dailyStreak < 3652) {//10 years is approximately 3,652 days
      ...
      if ((task.streak === 0 || task.streak === 1) || task.repeatFrequency === 5) {
      ...
      const xpEarned: number = Math.max(Math.floor(task.difficulty * task.priority * dateMultiplier * repeatMultiplier * streakMultiplier * dailyStreakMultiplier * dayTasksMultiplier), 1); //get at least 1 XP when the task is completed
      ...
      const pointsEarned: number = Math.max(Math.floor(task.difficulty * task.priority * dateMultiplier * repeatMultiplier * streakMultiplier * dailyStreakMultiplier * dayTasksMultiplier * levelMultiplier * tasksMultiplier * activeTasksMultiplier), 1); //get at least 1 point when the task is completed
      ...
      if (pointsEarned > state.user.bestScoreEarned) {//if points earned is greaedr than best score earned
      ...
      state.user.level = Math.max(1, Math.floor(Math.pow(state.user.xp, 1 / 3 + 5e-16))); //calculate level based on how many XP and set level to 1 if total XP is 0
    ...
    complete_Todo: (state, payload) => {
      ...
      if (Number(item.repeatFrequency) === 5) {//if task is a one-time only
        ...
        if (Number(item.repeatFrequency) === 1) {//if task repeat frequency is daily
          const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setDate(new Date(item.originalDueDate + " 23:59:59.999").getDate() + item.timesCompleted * item.repeatOften)); //get new due date
        ...
        } else if (Number(item.repeatFrequency) === 2) {//if task repeat frequency is weekly
          const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setDate(new Date(item.originalDueDate + " 23:59:59.999").getDate() + item.timesCompleted * item.repeatOften * 7));
        ...
        } else if (Number(item.repeatFrequency) === 3) {//if task repeat frequency is monthly
          const monthsAfter: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setMonth(new Date(item.originalDueDate + " 23:59:59.999").getMonth() +item.timesCompleted * item.repeatOften));
          if (monthsAfter.getMonth() !== (new Date(item.originalDueDate + " 23:59:59.999").getMonth() + item.timesCompleted * item.repeatOften) % 12) {//if task due date is more than days of the month, set to last day of month
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear(), new Date(item.originalDueDate + " 23:59:59.999").getMonth() + item.timesCompleted * item.repeatOften + 1, 0, 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          } else {
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear(), new Date(item.originalDueDate + " 23:59:59.999").getMonth() + item.timesCompleted * item.repeatOften, new Date(item.originalDueDate + " 23:59:59.999").getDate(), 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          }
        } else if (Number(item.repeatFrequency) === 4) {//if task repeat frequency is yearly
          const yearsAfter: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setFullYear(new Date(item.originalDueDate + " 23:59:59.999").getFullYear() + item.timesCompleted * item.repeatOften));
          if (yearsAfter.getMonth() !== new Date(item.originalDueDate + " 23:59:59.999").getMonth()) {//if task due date don't have leap year, set task due date to February 28
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear() + item.timesCompleted * item.repeatOften, new Date(item.originalDueDate + " 23:59:59.999").getMonth() + 1, 0, 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          } else {
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear() + item.timesCompleted * item.repeatOften, new Date(item.originalDueDate + " 23:59:59.999").getMonth(), new Date(item.originalDueDate + " 23:59:59.999").getDate(), 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          }
        }
    ...
    delete_Todo: (state, payload) => {
      ...
      let deleteTask;
      if (!state.todos[index].completed) {//don't ask for confirmation when one-time task is completed
      ...
      if (deleteTask || state.todos[index].completed) {//delete task if one-time task is completed when the delete button is clicked or when user confirms deletion alert
