...
interface Todo {//todos task interface
...
export default createStore({
  ...
  mutations: {
    ...
    updateXp: (state, payload) => {
      const task: Todo = state.todos.find((todo: { newId: number }) => todo.newId === payload) as Todo;
      const daysToDue: number = Math.round((Number(new Date(task.dueDate + " 23:59:59.999")) - Number(new Date().setHours(23, 59, 59, 999))) / (1000 * 60 * 60 * 24));//calculate the number of days until the task is due
        let dateMultiplier: number;
        if (daysToDue < 0) {//if the task is overdue, XP and score multiplier is less than 1 that decreases over time when the task is overdue
          dateMultiplier = -2 / (daysToDue - 1);
        } else if (daysToDue === 0) {//if the task is due today, XP and score multiplier bonus increases more than 2 based on the time the task is completed
          dateMultiplier = 4 / (1 + (Number(new Date().setHours(23, 59, 59, 999)) - Number(new Date())) / (1000 * 24 * 60 * 60));
        } else {//else XP and score multiplier bonus increases (more than 1) when the task gets closer to due date
      ...
      const activeTasks: number = state.todos.filter((taskList) => !taskList.isCompleted).length;//calculate the number of active tasks (tasks that are not completed) using Array.filter
      const overdueTasks: number = state.todos.filter((taskList) => new Date(new Date(new Date().setMinutes(new Date().getMinutes() - new Date().getTimezoneOffset())).toISOString().split("T")[0] + " 23:59:59.999") >= new Date(taskList.dueDate + " 23:59:59.999")).length;//calculate the number of overdue tasks (tasks after the due date)
      ...
      if (Number(task.repeatInterval) === 1) {//if task repetition interval is daily
        if (task.repeatEvery < 7) {//7 days is 1 week
        ...
        } else if (task.repeatEvery < 30) {//approximately 30 days is 1 month
        ...
        } else if (task.repeatEvery < 365) {//approximately 365 days is 1 year
      ...
      } else if (Number(task.repeatInterval) === 2) {//if task repetition interval is weekly
        if (task.repeatEvery < 4) {//approximately 4 weeks is 1 month
        ...
        } else if (task.repeatEvery < 52) {//approximately 52 weeks is 1 year
      ...
      } else if (Number(task.repeatInterval) === 3) {//if task repetition interval is monthly
        if (task.repeatEvery < 12) {//12 months is 1 year
      ...
      } else if (Number(task.repeatInterval) === 4) {//if task repetition interval is yearly
        repeatMultiplier = 5 - 1 / task.repeatEvery; //4x XP multiplier for yearly tasks (1 year) to 5x XP multiplier for one-time tasks
      } else {//if task repetition interval is one-time
        repeatMultiplier = 5; //get 5x XP multiplier for one-time tasks
      }
      //calculate task streak
      if (daysToDue < 0) {//if task is overdue
        task.streak = 0;//reset task streak to 0
      }
      else {//if the task is completed before due date (not overdue)
        task.streak++;//increase task streak
      }
      //calculate daily streak
      if (state.user.lastCompletionDate === undefined || new Date(new Date().setDate(new Date().getDate() - 1)) > new Date(state.user.lastCompletionDate + " 23:59:59.999")) {//if user last completion date is before yesterday or undefined (no user task completed yet)
        state.user.dailyStreak = 1;//reset daily streak to 1
      }
      else if ((Number(new Date(new Date().setHours(23, 59, 59, 999))) - Number(new Date(state.user.lastCompletionDate + " 23:59:59.999"))) === (1000 * 60 * 60 * 24)) {//if user last completion date is yesterday
        state.user.dailyStreak++;//increase daily streak
      }
      //calculate the number of tasks completed in a day
      if (state.user.lastCompletionDate === undefined || (Number(new Date(state.user.lastCompletionDate + " 23:59:59.999")) !== Number(new Date(new Date().setHours(23, 59, 59, 999))))) {//if new day has passed
      ...
      else if (state.user.dailyStreak < 7) {//1 week is 7 days
      ...
      else if (state.user.dailyStreak < 14) {//2 weeks are 14 days
      ...
      else if (state.user.dailyStreak < 30) {//1 month is approximately 30 days
      ...
      else if (state.user.dailyStreak < 60) {//2 months are approximately 60 days
      ...
      else if (state.user.dailyStreak < 90) {//3 months are approximately 90 days
      ...
      else if (state.user.dailyStreak < 180) {//6 months are approximately 180 days
      ...
      else if (state.user.dailyStreak < 365) {//1 year is approximately 365 days
      ...
      else if (state.user.dailyStreak < 730) {//2 years are approximately 730 days
      ...
      else if (state.user.dailyStreak < 1461) {//4 years are approximately 1,461 days
      ...
      else if (state.user.dailyStreak < 3652) {//10 years are approximately 3,652 days
      ...
      const previousCompletionDate: string | undefined = state.user.lastCompletionDate;//set previous completion date to last completion date
      //set the last completion date to today
      state.user.lastCompletionDate = new Date(new Date().setMinutes(new Date().getMinutes() - new Date().getTimezoneOffset())).toISOString().split("T")[0];
      const daysSinceLastCompletion: number = Math.round((Number(new Date(state.user.lastCompletionDate + '23:59:59.999')) - Number(new Date(previousCompletionDate + '23:59:59.999'))) / (1000 * 60 * 60 * 24));//calculate days since task last completion (inactivity)
      //check if at least 1 day of inactivity
      if (daysSinceLastCompletion >= 1) {
        //repeat for each day of inactivity
        for (let i = 0; i < daysSinceLastCompletion; i++) {
          state.user.rating -= Math.max((Math.sqrt(Math.max(state.user.rating, 0)) * (1 + Math.log(Math.max(i + 1, 1))) * (1 + Math.log(Math.max(overdueTasks + 1, 1)))), 0);//decrease user rating for each day of inactivity
          state.user.rating = Math.max(state.user.rating, 0);//make sure rating is not below 0
        }
      }
      //calculate task streak XP multiplier
      if ((task.streak === 0 || task.streak === 1) || task.repeatInterval === 5) {//if task streak is 0 or 1 or task repeat interval is one-time
      ...
      if (state.user.tasksCompletedToday === 0 || state.user.tasksCompletedToday === 1){
        ...
        dayTasksMultiplier = 2.5 + 0.025 * (state.user.tasksCompletedToday - 20);//2.5x multiplier plus 0.025x multiplier for each task completed in a day from 20 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 100){
        dayTasksMultiplier = 3.25 + 0.02 * (state.user.tasksCompletedToday - 50);//3.25x multiplier plus 0.02x multiplier for each task completed in a day from 50 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 200){
        dayTasksMultiplier = 4.25 + 0.01 * (state.user.tasksCompletedToday - 100);//4.25x multiplier plus 0.01x multiplier for each task completed in a day from 100 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 500){
        dayTasksMultiplier = 5.25 + 0.005 * (state.user.tasksCompletedToday - 200);//5.25x multiplier plus 0.005x multiplier for each task completed in a day from 200 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 1000){
        dayTasksMultiplier = 6.75 + 0.0025 * (state.user.tasksCompletedToday - 500);//6.75x multiplier plus 0.0025x multiplier for each task completed in a day from 500 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 2000){
        dayTasksMultiplier = 8 + 0.002 * (state.user.tasksCompletedToday - 1000);//8x multiplier plus 0.002x multiplier for each task completed in a day from 1,000 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 5000){
        dayTasksMultiplier = 10 + 0.001 * (state.user.tasksCompletedToday - 2000);//10x multiplier plus 0.001x multiplier for each task completed in a day from 2,000 tasks completed in a day
      }
      else if (state.user.tasksCompletedToday < 10000){
        dayTasksMultiplier = 13 + 0.0006 * (state.user.tasksCompletedToday - 5000);//13x multiplier plus 0.0006x multiplier for each task completed in a day from 5,000 tasks completed in a day
      }
      else {
      ...
      if (state.user.totalTasksCompleted === 0 || state.user.totalTasksCompleted === 1) {
        ...
        tasksMultiplier = 14.5 + 0.0003 * (state.user.totalTasksCompleted - 10000);//14.5x task score multiplier from 10,000 total tasks completed plus 0.0003x task score multiplier for each task completed
      }
      else if (state.user.totalTasksCompleted < 50000) {
        tasksMultiplier = 17.5 + 0.00025 * (state.user.totalTasksCompleted - 20000);//17.5x task score multiplier from 20,000 total tasks completed plus 0.00025x task score multiplier for each task completed
      }
      else if (state.user.totalTasksCompleted < 100000) {
        tasksMultiplier = 25 + 0.0001 * (state.user.totalTasksCompleted - 50000);//25x task score multiplier from 50,000 total tasks completed plus 0.0001x task score multiplier for each task completed
      }
      else if (state.user.totalTasksCompleted < 200000) {
        tasksMultiplier = 30 + 0.00006 * (state.user.totalTasksCompleted - 100000);//30x task score multiplier from 100,000 total tasks completed plus 0.00006x task score multiplier for each task completed
      }
      else if (state.user.totalTasksCompleted < 500000) {
        tasksMultiplier = 36 + 0.00003 * (state.user.totalTasksCompleted - 200000);//36x task score multiplier from 200,000 total tasks completed plus 0.00003x task score multiplier for each task completed
      }
      else if (state.user.totalTasksCompleted < 1000000) {
        tasksMultiplier = 45 + 0.00002 * (state.user.totalTasksCompleted - 500000);//45x task score multiplier from 500,000 total tasks completed plus 0.00002x task score multiplier for each task completed
      }
      else {
      ...
      const rankXpEarned: number = Math.max(Math.floor((dateMultiplier - 1) ** 2 * 100 * Math.max(task.rank, 1) * Math.max(task.streak, 1) * repeatMultiplier * (1 + rankMultiplier / 10)), 1);//get at least 1 rank XP when the task is completed
      task.rankXp += rankXpEarned;//increase rank XP based on task due date, task streak and task rank
      const rankLevel: number = Math.max(Math.floor(Math.pow((task.rankXp + 0.5) / 100, 1 / 4)), 1);//update rank level
      task.rank = rankLevel;//set task rank level
      task.rankProgress = (((task.rankXp / 100) - Math.pow(task.rank === 1 ? 0 : task.rank, 4)) / (Math.pow(task.rank + 1, 3) - Math.pow(task.rank === 1 ? 0 : task.rank, 4))) * 100; //calculate rank level progress and if level is 1 set rank level at the start of level 1 to 0 XP
      const xpEarned: number = Math.max(Math.floor(task.difficulty * task.priority * dateMultiplier * repeatMultiplier * streakMultiplier * dailyStreakMultiplier * dayTasksMultiplier * (1 + task.rank / 10) * (1 + rankMultiplier / 10)), 1); //get at least 1 XP when the task is completed
      ...
      state.user.rating += Math.max((10 + Math.log(Math.max(state.user.rating + 100, 100)) ** 2) * repeatMultiplier * dateMultiplier < 1 ? (1 - dateMultiplier) : (dateMultiplier - 1) / Math.max(state.user.tasksCompletedToday, 1), 0);//get the amount of rating poings earned based on user rating, task repeat multiplier and number of tasks completed today
      state.user.rating = Math.max(state.user.rating, 0);//make sure user rating is not below 0
      const pointsEarned: number = Math.max(Math.floor(task.difficulty * task.priority * dateMultiplier * repeatMultiplier * streakMultiplier * dailyStreakMultiplier * dayTasksMultiplier * levelMultiplier * tasksMultiplier * activeTasksMultiplier * (1 + task.rank / 10) * (1 + rankMultiplier / 10)), 1); //get at least 1 point when the task is completed
      ...
      if (pointsEarned > state.user.bestScoreEarned) {//if points earned are greater than the best score earned
        state.user.bestScoreEarned = pointsEarned;//set the best score earned to points earned when the task is completed
      }
      alert(`Task ${task.task} completed!\nYou earned ${xpEarned.toLocaleString("en-US")} XP!\nYou earned ${pointsEarned.toLocaleString("en-US")} point${pointsEarned === 1 ? "" : "s"}!`);//alert user to show how many XP they earned and points earned after completing the task
      //check if user has leveled up
      const userLevel: number = state.user.level;//set userLevel variable before calculating user level state
      state.user.level = Math.max(1, Math.floor(Math.cbrt(state.user.xp + 0.5))); //calculate level based on how many XP and set level to 1 if total XP is 0
      if (state.user.level > userLevel) {
        alert(`Level Up!\nYou are now level ${state.user.level.toLocaleString("en-US")}!`);//alert user when user levels up
      }
      state.user.progress = ((state.user.xp - Math.pow(state.user.level === 1 ? 0 : state.user.level, 3)) / (Math.pow(state.user.level + 1, 3) - Math.pow(state.user.level === 1 ? 0 : state.user.level, 3))) * 100; //calculate level progress and if level is 1 set total XP at the start of level 1 to 0 XP
    },
    ...
    complete_Todo: (state, payload) => {
       ...
       */
      const item: Todo = state.todos.find((todo: { newId: number }) => todo.newId === payload) as Todo;
      if (Number(item.repeatInterval) === 5) {//if the task is a one-time only
        ...
        if (Number(item.repeatInterval) === 1) {//if task repeat interval is daily
          const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setDate(new Date(item.originalDueDate + " 23:59:59.999").getDate() + item.timesCompleted * item.repeatEvery)); //get a new due date
          const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset())); //convert to local timezone
          item.dueDate = adjustedNewDueDate.toISOString().split("T")[0]; //convert due date to YYYY-MM-DD string
        } else if (Number(item.repeatInterval) === 2) {//if task repeat interval is weekly
          const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setDate(new Date(item.originalDueDate + " 23:59:59.999").getDate() + item.timesCompleted * item.repeatEvery * 7));
          const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
          item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
        } else if (Number(item.repeatInterval) === 3) {//if task repeat interval is monthly
          const monthsAfter: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setMonth(new Date(item.originalDueDate + " 23:59:59.999").getMonth() +item.timesCompleted * item.repeatEvery));
          if (monthsAfter.getMonth() !== (new Date(item.originalDueDate + " 23:59:59.999").getMonth() + item.timesCompleted * item.repeatEvery) % 12) {//if task due date is more than days of the month, set to last day of month
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear(), new Date(item.originalDueDate + " 23:59:59.999").getMonth() + item.timesCompleted * item.repeatEvery + 1, 0, 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          } else {
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear(), new Date(item.originalDueDate + " 23:59:59.999").getMonth() + item.timesCompleted * item.repeatEvery, new Date(item.originalDueDate + " 23:59:59.999").getDate(), 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          }
        } else if (Number(item.repeatInterval) === 4) {//if task repeat interval is yearly
          const yearsAfter: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").setFullYear(new Date(item.originalDueDate + " 23:59:59.999").getFullYear() + item.timesCompleted * item.repeatEvery));
          if (yearsAfter.getMonth() !== new Date(item.originalDueDate + " 23:59:59.999").getMonth()) {//if task due date don't have leap year, set task due date to February 28
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear() + item.timesCompleted * item.repeatEvery, new Date(item.originalDueDate + " 23:59:59.999").getMonth() + 1, 0, 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          } else {
            const newDueDate: Date = new Date(new Date(item.originalDueDate + " 23:59:59.999").getFullYear() + item.timesCompleted * item.repeatEvery, new Date(item.originalDueDate + " 23:59:59.999").getMonth(), new Date(item.originalDueDate + " 23:59:59.999").getDate(), 23, 59, 59, 999);
            const adjustedNewDueDate: Date = new Date(newDueDate.setMinutes(newDueDate.getMinutes() - newDueDate.getTimezoneOffset()));
            item.dueDate = adjustedNewDueDate.toISOString().split("T")[0];
          }
        }
    ...
    delete_Todo: (state, payload) => {
      ...
      let deleteTask;
      if (!state.todos[index].isCompleted) {//don't ask for confirmation when one-time task is completed
        deleteTask = confirm(`Do you want to delete the task ${state.todos[index].task}?\nThis action cannot be undone.`) as boolean; //ask user to confirm task deletion
      }
      if (deleteTask || state.todos[index].isCompleted) {//delete task if one-time task is completed when the deleted button is clicked or when user confirms deletion alert
